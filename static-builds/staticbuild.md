...
sudo find ${DISTNAME}/bin -type f -executable -exec ../apixcash/contrib/devtools/split-debug.sh {} {} {}.dbg \;
sudo find ${DISTNAME}/lib -type f -exec ../apixcash/contrib/devtools/split-debug.sh {} {} {}.dbg \;
...
make dist
SOURCEDIST=`echo apixcash-*.tar.gz`
mkdir -p temp
cd temp
tar xf ../$SOURCEDIST
find apixcash-* | sort | tar --no-recursion --mode='u+rw,go+r-w,a+X' --owner=0 --group=0 -c -T - | gzip -9n > ../$SOURCEDIST
cd ~/apixcash
...
make clean
export PATH=$PATH_orig
#transfer ~/sign/apixcash-*-win*-unsigned.tar.gz to the signing machine
```
...
#transfer apixcash-*-win*-unsigned.tar.gz to ~/win64/  
...
#transfer apixcash-*-win*-setup.exe back to the Ubuntu18 build machine to the folder ~/release (to shasum with the rest of the releases)
```
...
cd ~/
sudo rm -rf win32
cp -rf apixcash/contrib/windeploy ~/sign/win32
...
#transfer apixcash-*-win*-unsigned.tar.gz to ~/win32/  
tar xf $DISTNAME-win*-unsigned.tar.gz
rm $DISTNAME-win*-unsigned.tar.gz
./detached-sig-create.sh -key /path/to/codesign.pvk
#Enter the passphrase for the key when prompted
...
~/apixcash/contrib/macdeploy/extract-osx-sdk.sh
...
git clone https://github.com/apixcashproject/apixcash
mkdir ~/apixcash/depends/SDKs
#transfer MacOSX10.11.sdk.tar.gz to the folder apixcash/depends/SDKs
cd apixcash/depends/SDKs && tar -xf MacOSX10.11.sdk.tar.gz 
...
cd ~/apixcash
make deploy
$PWD/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Apixcash-Core.dmg" ~/release/unsigned/$DISTNAME-osx-unsigned.dmg
...
make clean
export PATH=$PATH_orig
# Transfer ~/sign/apixcash-*-osx*-unsigned.tar.gz to a MacOS signing machine
```
...
cd ~/desktop
mkdir OSX
#transfer apixcash-*-osx-unsigned.tar.gz to ~/desktop/OSX
cd OSX
