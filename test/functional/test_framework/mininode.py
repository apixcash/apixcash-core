...
"""Apixcash P2P network half-a-node.

This python code was modified from ArtForz' public domain  half-a-node, as
found in the mini-node branch of http://github.com/jgarzik/pynode.

NodeConn: an object which manages p2p connectivity to a apixcash node
...
from test_framework.util import hex_str_to_bytes, bytes_to_hex_str, wait_until, x16_hash_block
...
# Objects that map to apixcashd objects, which can be serialized/deserialized
class CAddress:
    def __init__(self):
        self.nServices = 1
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in apixcashd
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in apixcashd indicates these should be deserialized as blocks
...
class NodeConnCB:
    """Callback and helper functions for P2P connection to a apixcashd node.
...
class NodeConn(asyncore.dispatcher):
    ...
    def __init__(self, dstaddr, dstport, rpc, callback, net="regtest", services=NODE_NETWORK, send_version=True):
        ...
        logger.info('Connecting to Apixcash Node: %s:%d' % (self.dstaddr, self.dstport))
