...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for APX
    ...
    if (p_rewardSnapshot.strDistributionAsset != "APX") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is APX with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using APX differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "APX") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Apixcash address (already validated during ownership snapshot creation)
