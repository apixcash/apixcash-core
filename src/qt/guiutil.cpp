...
#include "apixcashaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Apixcash address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new ApixcashAddressEntryValidator(parent));
    widget->setCheckValidator(new ApixcashAddressCheckValidator(parent));
}
...
bool parseApixcashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no apixcash: URI
    if(!uri.isValid() || uri.scheme() != QString("apixcash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ApixcashUnits::parse(ApixcashUnits::APX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseApixcashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert apixcash:// to apixcash:
    //
    //    Cannot handle this later, because apixcash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("apixcash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "apixcash:");
    }
    QUrl uriInstance(uri);
    return parseApixcashURI(uriInstance, out);
}

QString formatApixcashURI(const SendCoinsRecipient &info)
{
    QString ret = QString("apixcash:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ApixcashUnits::format(ApixcashUnits::APX, info.amount, false, ApixcashUnits::separatorNever));
        paramCount++;
    }
...
bool openApixcashConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(APIXCASH_CONF_FILENAME);
    ...
    /* Open apixcash.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Apixcash.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Apixcash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Apixcash (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Apixcash*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "apixcash.desktop";
    return GetAutostartDir() / strprintf("apixcash-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a apixcash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Apixcash\n";
        else
            optionFile << strprintf("Name=Apixcash (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the apixcash app
...
bool GetStartOnSystemStartup()
{
    CFURLRef apixcashAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (apixcashAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, apixcashAppUrl);

    CFRelease(apixcashAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add apixcash app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, apixcashAppUrl, nullptr, nullptr);
    }
