...
#include "apixcashgui.h"

#include "apixcashunits.h"
...
const std::string ApixcashGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ApixcashGUI::DEFAULT_WALLET = "~Default";

ApixcashGUI::ApixcashGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ApixcashGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Apixcash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and apixcash: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to APX addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Apixcash addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Apixcash addresses"));
    ...
    openAction->setStatusTip(tr("Open a apixcash: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Apixcash command-line options").arg(tr(PACKAGE_NAME)));
...
void ApixcashGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void ApixcashGUI::createToolBars()
{
    if(walletFrame)
    {
        /** APX START */
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* apixcashLabelLayout = new QVBoxLayout(toolbarWidget);
        apixcashLabelLayout->addWidget(label);
        apixcashLabelLayout->addWidget(toolbar);
        apixcashLabelLayout->setDirection(QBoxLayout::TopToBottom);
        apixcashLabelLayout->addStretch(1);
...
void ApixcashGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ApixcashGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ApixcashGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ApixcashGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ApixcashGUI::setWalletActionsEnabled(bool enabled)
{
...
void ApixcashGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ApixcashGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ApixcashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ApixcashGUI::optionsClicked()
{
...
void ApixcashGUI::showDebugWindow()
{
...
void ApixcashGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ApixcashGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void ApixcashGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ApixcashGUI::openClicked()
{
...
void ApixcashGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ApixcashGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ApixcashGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ApixcashGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ApixcashGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ApixcashGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** APX START */
void ApixcashGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void ApixcashGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void ApixcashGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void ApixcashGUI::gotoRestrictedAssetsPage()
{
...
void ApixcashGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Apixcash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ApixcashGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ApixcashGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ApixcashGUI::updateHeadersSyncProgressLabel()
{
...
void ApixcashGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ApixcashGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Apixcash"); // default title
...
void ApixcashGUI::changeEvent(QEvent *e)
{
...
void ApixcashGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ApixcashGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ApixcashGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "APX")
        msg += tr("Amount: %1\n").arg(ApixcashUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ApixcashUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void ApixcashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ApixcashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ApixcashGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ApixcashGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ApixcashGUI::setHDStatus(int hdEnabled)
{
...
void ApixcashGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ApixcashGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ApixcashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ApixcashGUI::detectShutdown()
{
...
void ApixcashGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ApixcashGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ApixcashGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ApixcashGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ApixcashGUI::subscribeToCoreSignals()
{
...
void ApixcashGUI::unsubscribeFromCoreSignals()
{
...
void ApixcashGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ApixcashGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ApixcashUnits::Unit> units = ApixcashUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ApixcashUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(ApixcashUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(ApixcashUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (ApixcashUnits::Unit u : ApixcashUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ApixcashUnits::name(u)), this);
...
{
    setText(ApixcashUnits::name(newUnits));
}
