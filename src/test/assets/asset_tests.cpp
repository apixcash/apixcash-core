...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of APIXCASH NOT allowed
        BOOST_CHECK(!IsAssetNameValid("APX", type));
        BOOST_CHECK(!IsAssetNameValid("APIXCASH", type));
        BOOST_CHECK(!IsAssetNameValid("APIXCASH", type));

        //- Versions of APIXCASH ALLOWED
        BOOST_CHECK(IsAssetNameValid("APIXCASH.COIN", type));
        BOOST_CHECK(IsAssetNameValid("APIXCASH_COIN", type));
        BOOST_CHECK(IsAssetNameValid("APXSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERAPX", type));
        BOOST_CHECK(IsAssetNameValid("APIXCASHSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEAPIXCASH", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_APIXCASHS", type));
        BOOST_CHECK(IsAssetNameValid("SEAPXOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("APIXCASH", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("APIXCASH", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
