...
The RPM spec file provided here is for Apixcash-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://apixcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Apixcash git repository](https://github.com/ApixcashProject/Apixcash) but are not part of
...
Currently only `Source10` is used, for the example `apixcash.conf` file.
...
in the apixcash git repository. At present only an SVG version of the Apixcash
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the apixcash
...
needed when 0.12.1 is released, a proper fix is already in the Apixcash git
master branch.
...
idea to build Apixcash Core against a static version of that library compiled
...
The `apixcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `apixcash-server` package. At this point, most RPM based
...
The `apixcash-server` package should have SELinux support. How to properly do
...
Apixcash-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/apixcash/). He has been
packaging Apixcash for Fedora at least since 2012.
...
4. I renamed the `apixcash` package that contains the Qt GUI to `apixcash-core` as
that appears to be how the general population refers to it, in contrast to
`apixcash-xt` or `apixcash-classic`. I wanted to make sure the general population
